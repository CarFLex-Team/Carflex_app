...
export async function GET(req: Request) {
  try {
    ...
    const stmt = db.prepare(
      `SELECT id, name as title, price, mileage_value  AS odometer, image as image_src, url as ad_link, sortingDate,  description FROM kjiji ORDER BY sortingDate LIMIT  ?`
    );
    ...
    console.error("GET /api/kijiji error", err);
